...
// // //3. หา average total
// console.log("Bills AVG: ", totalBills/ bills.length);




// // //4. หาจำนวนเมมเบอว่ามีทั้งหมดกี่คน
...
const uniqueMember = members.reduce((acc,item) => {
...
function totalLocation(b) {
    let bkkcount = 0;
    let bkktotal = 0;
    let nancount = 0;
    for (i = 0; i < b.length; i++) {
      if (b[i].location == 'Bangkok') {
        bkkcount += 1
        bkktotal += b[i].total;
      } else if (b[i].location == 'Nan') { 
        nancount++;
      }
    }
    console.log("Bangkok = ", bkkcount)
    console.log("Bangkok Total = ", bkktotal)
